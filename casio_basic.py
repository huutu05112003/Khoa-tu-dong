...
def calculate():
    ...
    if equation != "":
        try:
            result = eval(equation) # Ham tu tinh bieu thuc duoi dang chuoi
            result = '{} = {}'.format(equation, result)
            equation = ""
        except:
            result = "error"
            equation = ""
...
Button(root, text="=", width=5, height=3, font=('arial', 30, 'bold'), bd= 1, fg = "#fff",bg="#fe8037", command= lambda: calculate()).place(x=430, y=400) 
